...
(defn- scroll-into-view [id]
  (js/setTimeout
   #(do
      (.scrollIntoView
       (.getElementById js/document (str "item-card-" id))
       (clj->js
        {:behavior "instant"
         :block "center"
         :inline "nearest"}))
...
(defn- items-list-component [*state]
  [:ul.cards
   (map-indexed
    (fn [idx item]
      [:<>
       {:key (:id item)}
       [items-list-items/regular-items-list-item-component 
        *state item idx {:allow-delete-on-right-click? true
  ...
  (fn [*state]
    (let [state @*state]
      [:<>
       ...
       [:div.scrollable
        {:class (when (= :items (:active-search state)) :search-active)}
        (when (not= :description (:modal state))
          [items-list-component *state])]])))
