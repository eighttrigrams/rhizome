...
(defn get-title-el [] (.getElementById js/document "search-input"))


(defn- item-creation-permitted?
  [{{{{{:keys [secondary-contexts-unassigned-selected search-mode]} :current} :views} :data}
      :selected-item}]
  (and (not secondary-contexts-unassigned-selected) (or (nil? search-mode) (= 0 search-mode))))
...
(defn handle-keys
  ...
  (handle-keys*
    (fn [code ctrl-pressed? meta-pressed? alt-pressed? shift-pressed? e]
      (let [{:keys [active-search selected-item link-item link-context]} @*state
            in-notes-mode? (-> *state
                               deref
                               :selected-item
                               :data
        ...
        (when (= code "Enter")
          (.preventDefault e)
          (cond
            (and ctrl-pressed? meta-pressed? alt-pressed? (= :items active-search) selected-item)
              (actions/store-current-view! *state {:title (.-value (get-title-el))})
            (and (not (:link-context @*state))
            ...
            (and (= :items active-search)
                 ...
                 selected-item
                 (not link-item)
                 (or (and shift-pressed? (not alt-pressed?)) (= 0 (count (:items @*state))))
        ...
        (when (and (= "KeyV" code) alt-pressed? shift-pressed? (= :items active-search) selected-item)
          (.preventDefault e)
          (actions/toggle-vector-search-mode! *state))
        (when (and (= "KeyC" code) alt-pressed?)
        ...
        (when (= code "Escape")
          (cond (and alt-pressed? (common/something-to-deselect? *state))
                  (actions/deselect-secondary-contexts! *state)
                (or (not selected-item) selected-item) (actions/quit-search! *state)))))))
