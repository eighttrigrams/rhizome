...
(defn starts-with-pattern? [s pattern] (not (nil? (re-find (re-pattern (str "^" pattern)) s))))
...
(defn next-line-position
  ...
  (loop [i 0
    ...
    (let [has-reached-end-of-file? (= pos (count value))
      ...
      (if (or has-reached-end-of-file? has-encountered-newline?)
        (if has-reached-end-of-file? [i pos] [(inc i) (inc pos)])
        (recur (inc i) (inc pos))))))
  ...
  (let [start-i -1]
    (loop [i start-i
      ...
      (let [has-reached-beginning-of-file? (= position start-i)
            has-encountered-newline? (and (not (= i start-i)) (= (get value position) \newline))]
        (if (or has-reached-beginning-of-file? has-encountered-newline?)
          [i (inc position) (not has-reached-beginning-of-file?)]
          (recur (inc i) (dec position)))))))
...
(defn index-of-substr-or-end
  ...
  (loop [rst s
    ...
    (if (= i (count s))
      i
      (if-not (starts-with-pattern? rst pattern) (recur (apply str (rest rst)) (inc i)) i))))
...
(defn calc-rest
  [{value :value selection-end :selection-end}]
  (subs value selection-end (count value)))

(defn pull-r
  [{selection-end :selection-end :as state}]
  (assoc state :selection-start selection-end))

(defn pull-l
  [{selection-start :selection-start :as state}]
