...
(defn- _transform-state
  [command state direction]
  (case command
    :nop state
    :insert-tab (lowlevel/insert-tab state)
    :insert (lowlevel/insert state)
    :caret-right ((comp (if (= direction -1) h/pull-l h/pull-r) lowlevel/caret-right) state)
    :caret-left ((comp (if (= direction 1) h/pull-r h/pull-l) lowlevel/caret-left) state)
    :caret-up ((comp h/pull-l lowlevel/same-position-previous-line) state)
    :caret-down ((comp h/pull-r lowlevel/same-position-next-line) state)
    :caret-down-with-selection (if (= direction -1)
                                 ((comp h/flip lowlevel/same-position-next-line h/flip) state)
                                 (assoc (lowlevel/same-position-next-line state) :direction 1))
    :caret-up-with-selection (if (= direction 1)
    ...
    :caret-right-with-selection (if (= direction -1)
                                  ((comp h/flip lowlevel/caret-right h/flip) state)
                                  (assoc (lowlevel/caret-right state) :direction 1))
    :caret-left-with-selection (if (= direction 1)
                                 ((comp h/flip lowlevel/caret-left h/flip) state)
                                 (assoc (lowlevel/caret-left state) :direction -1))
    :word-part-right ((comp (if (= direction -1) h/pull-l h/pull-r) lowlevel/word-part-right) state)
    :word-part-left ((comp (if (= direction 1) h/pull-r h/pull-l) lowlevel/word-part-left) state)
    :move-selection-wordwise-right (if (= direction -1)
                                     ((comp h/flip lowlevel/word-part-right h/flip) state)
                                     (assoc (lowlevel/word-part-right state) :direction 1))
    :move-selection-wordwise-left (if (= direction 1)
                                    ((comp h/flip lowlevel/word-part-left h/flip) state)
                                    (assoc (lowlevel/word-part-left state) :direction -1))
    :sentence-right ((comp (if (= direction -1) h/pull-l h/pull-r) lowlevel/sentence-part-right)
                      state)
    :sentence-left ((comp (if (= direction 1) h/pull-r h/pull-l) lowlevel/sentence-part-left) state)
    :sentence-right-with-selection (if (= direction -1)
                                     ((comp h/flip lowlevel/sentence-part-right h/flip) state)
                                     (assoc (lowlevel/sentence-part-right state) :direction 1))
    :sentence-left-with-selection (if (= direction 1)
    ...
    :delete-wordwise-backward (lowlevel/delete-word-part-left state)
    :delete-wordwise-forward (lowlevel/delete-word-part-right state)
    :delete-sentence-wise-backward (lowlevel/delete-sentence-part-left state)
    :delete-sentence-wise-forward (lowlevel/delete-sentence-part-right state)
    :shift-enter (lowlevel/newline-after-current state)
    :alt-enter (lowlevel/newline-before-current state)))
    ...
    (fn transform-state [command state]
      (let [direction @direction:atom
            {selection-start :selection-start
             ...
             dir :direction
             :as new-state}
              (_transform-state command state direction)
