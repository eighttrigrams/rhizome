...
(defn insert-tab
  [{value :value selection-start :selection-start selection-end :selection-end :as state}]
  (if (not= selection-start selection-end)
    (-> state
        (assoc :value
                 (str (subs value 0 selection-start) "\t" (subs value selection-end (count value))))
        (assoc :selection-start (inc selection-start))
        (assoc :selection-end (inc selection-start)))
    (-> state
        (assoc :value (str (subs value 0 selection-start)
                           "\t"
...
(defn caret-left
  ...
  (let [selection-start (if (> selection-start 0) (- selection-start 1) selection-start)]
    (-> state
...
(defn caret-right
  [{value :value selection-start :selection-start selection-end :selection-end :as state}]
  (let [selection-end (if (< selection-end (count value)) (+ selection-end 1) selection-end)]
    (-> state
        (assoc :position-in-line selection-start)
        (assoc :selection-start selection-start)
        (assoc :selection-end selection-end))))

(defn word-part-right
  [{selection-start :selection-start selection-end :selection-end :as state}]
  (let [rest (h/calc-rest state)
        selection-end
          (+ selection-end
             (cond (h/starts-with-pattern? rest word-stop-pattern) 1
                   (h/starts-with-pattern? rest "[ ]") (h/index-of-substr-or-end rest "[^ ]")
                   :else (h/index-of-substr-or-end rest word-stop-pattern-incl-whitespace)))]
    (-> state
        (assoc :selection-start selection-start)
        (assoc :selection-end selection-end))))
...
(defn delete-right
  [fun]
  (fn [{value :value selection-end :selection-end :as state}]
  ...
  (let [rest (h/calc-rest state)
        selection-end (+ selection-end
                         (if (h/starts-with-pattern? rest sentence-stop-pattern)
                           1
                           (h/index-of-substr-or-end rest sentence-stop-pattern)))]
    (-> state
        (assoc :selection-start selection-start)
        (assoc :selection-end selection-end))))
...
(defn- find-position-in-line-starting-from-end-of-line
  [{value :value position-in-line :position-in-line} position-of-linebreak]
...
(defn same-position-previous-line
  ...
  (-> state
      ...
      (assoc :selection-start
               (let [[_ line-position previous-line-exists?]
                       (h/cursor-position-in-line value selection-start)]
                 (if-not previous-line-exists?
                   selection-start
                   (find-position-in-line-starting-from-end-of-line state (dec line-position)))))))

(defn find-position-in-line-starting-from-beginning-of-line
  [{value :value position-in-line :position-in-line} position-of-line]
...
(defn same-position-next-line
  [{value :value selection-end :selection-end :as state}]
  (let [state (-> state
                  (assoc :prevent-adjust-position-in-line true)
                  (assoc :selection-end (let [[_ next-line-position]
                                                (h/next-line-position value selection-end)]
