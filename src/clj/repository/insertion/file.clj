...
(defn- get-additional-context-ids
  [db additional-context-titles]
  (let [additional-context-ids (mapv #(common/get-item-or-throw-error db %)
                                 additional-context-titles)]
...
(defn ingest
  ...
  (let [classification (classify file-name)
        ...
        context-ids-set (conj (set/union context-ids-set (into #{} additional-context-ids))
                              files-context-id)
        resource-links (merge {:file file-name}
                              (when (some #{"Image"} classification) {:image file-name}))]
