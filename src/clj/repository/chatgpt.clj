...
(defn form-params
  ...
  {:model (:model (configuration))
   :messages [{:role "system" :content (:prompt (configuration))} {:role "user" :content content}]})

(defn get-summary
  [content]
  (log/info (str "Using ChatGPT configuration: " (dissoc (configuration) :token)))
  (when-let [token (:token (configuration))]
    (try (let [parsed-response-body (json/parse-string
                                      (:body (http/post "https://api.openai.com/v1/chat/completions"
