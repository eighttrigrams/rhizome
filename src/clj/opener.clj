...
(defn- open-in-obsidian [filepath]
  (try
    ;; First try with vault specified
    (let [vault-name "Default%20Vault"
          relative-path "tracker.tmp.md"
          obsidian-uri (str "obsidian://open?vault=" vault-name "&file=" (java.net.URLEncoder/encode relative-path "UTF-8"))]
...
(defn create-obsidian-temp-file [item]
    ...
    (when (.exists (io/file filepath))
      (let [content (slurp filepath)
            ...
            desc-start-idx (loop [idx 2]
                            (if (>= idx (count lines))
                              idx
                              (if (str/blank? (nth lines idx))
                                (inc idx)
                                (recur (inc idx)))))
            description (if (< desc-start-idx (count lines))
