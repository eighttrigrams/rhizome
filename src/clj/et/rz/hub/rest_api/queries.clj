(ns et.rz.hub.rest-api.queries
  (:require [et.rz.hub.db :as db]
            ...
            [et.rz.hub.semsearch.query :as semsearch]
            [et.rz.hub.rest-api.util :refer [json-response item->api parse-int-opt parse-ids-csv]]))
...
;; et.rz.hub.repository.deletion, and hardcoded here for the same reason.
...
(defn- content-type-for
  "The content type a filename's suffix implies.
   ...
   under `images` (et.rz.hub.repository.insertion.file) -- and an old row can hold anything
...
(defn ^:no-describe describe
  ...
  vars in et.rz.hub.rest-api.queries and et.rz.hub.rest-api.mutations that carry a docstring; vars
  ...
  (json-response
    {:conventions global-conventions
     :skill @skill-md
     :endpoints (->> ['et.rz.hub.rest-api.queries 'et.rz.hub.rest-api.mutations]
