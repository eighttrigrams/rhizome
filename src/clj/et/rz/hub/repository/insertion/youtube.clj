(ns et.rz.hub.repository.insertion.youtube
  (:require [clojure.string :as str]
            [cambium.core :as log]
            [cheshire.core :as json]
            [clj-http.client :as http]
            [et.rz.hub.ds :as datastore]
            [et.rz.hub.repository.insertion.common :as common]
            [et.rz.utils.url :as url]
            [et.rz.hub.upload :as upload]
            et.rz.hub.scrapers.youtube))

(defn query
  [url]
  (json/parse-string (:body (http/get (str "https://www.youtube.com/oembed?"
                                           (url/make-query-string {"format" "json" "url" url}))))
                     true))
...
(defn- create-channel-or-take-existing
  ...
  (let [channel-handle-simple (str/replace author_url "https://www.youtube.com/" "")
        ...
        channel-id (:id (datastore/get-item-by-path db
                                                    "data->'resource-links'->>'youtube-channel'"
                                                    author_url))
...
(defn- calc-urls
  [url]
  (if (or (re-matches #"https://youtube.com/shorts/.*" url)
          (re-matches #"https://www.youtube.com/shorts/.*" url))
    (let [url (-> url
                  (str/replace "https://youtube.com/shorts/" "https://www.youtube.com/shorts/"))
    ...
    (let [url (if (str/includes? url "youtu.be")
                (str/replace (url/pick-query-params url []) "youtu.be/" "www.youtube.com/watch?v=")
                (url/pick-query-params url ["v"]))]
      [url url])))

(comment
  (calc-urls "https://www.youtube.com/watch?v=adkfslafj&m=13"))

(defn ingest
  [db url context-ids-set _]
  (let [[url store-url] (calc-urls url)
        {:keys [title author_name author_url] :as _response} (query url)
        _ (when-not (and (seq title) (seq author_name) (seq author_url))
        ...
        channel-id (create-channel-or-take-existing db author_name author_url youtube-channels-id)
        existing-item
          (datastore/get-item-by-path db "data->'resource-links'->>'youtube-video'" store-url)
        image (:image (et.rz.hub.scrapers.youtube/get-video url))]
    (if (:id existing-item)
      ...
      (let [item (common/insert-item db
            ...
            id (:id item)]
        (when image
          (try (upload/upload-preview-file db {:tempfile image} id "false")
               (catch Exception e
                 (log/error
                   (str "problem while trying to create preview image for youtube video. message"
                        (.getMessage e))))))
        item))))
