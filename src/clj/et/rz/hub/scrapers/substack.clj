(ns et.rz.hub.scrapers.substack
  (:require et.rz.utils
            ...
            et.rz.hub.scrapers.substack.common))
...
(defn- convert-date
  ...
  (let [[month day year] (filter #(not-empty %) (str/split date #"[\s,]"))]
    [(str year "-" (et.rz.hub.scrapers.substack.common/convert-month month) "-" day) year]))
...
(defn podcast-episode?
  ...
  (and (extract-date-for-pods tree)
       ...
       (try (et.rz.hub.scrapers.common/get-name tree "twitter:player") true (catch Exception _e false))))

(defn get-post
  [url extract-content]
  (let [tree (html/as-hickory (html/parse (:body (http/get url))))
        title (et.rz.hub.scrapers.common/get-property tree "og:title")
        subtitle (et.rz.hub.scrapers.common/get-property tree "og:description")
    ...
    {:title (subs (str title " - " subtitle) 0 (min 255 (count (str title " - " subtitle))))
     ...
     :content (-> tree
                  extract-content
                  et.rz.hub.scrapers.common/extract-text)
