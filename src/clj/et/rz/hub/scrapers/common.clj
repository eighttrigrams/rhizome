...
(defn extract-text
  ...
  (str/join (doall (reduce (fn [acc val]
                             (cond (string? val) (concat acc [val])
                                   (and (= :element (:type val)) (:content val))
...
(defn get-post
  ...
  (let [tree (html/as-hickory (html/parse (:body (http/get url))))
        title (first (:content (first (select/select (select/tag "title") tree))))]
    [title
     (-> tree
         extract-content
         extract-text)]))
