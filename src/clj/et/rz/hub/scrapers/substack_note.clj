(ns et.rz.hub.scrapers.substack-note
  (:require et.rz.utils
  ...
  (let [title-val (-> (select/select
    ...
    (when title-val
      (let [[date-and-month year] (str/split title-val #",")
            ...
            month (et.rz.hub.scrapers.substack.common/convert-month month)
...
(defn get-date
  ...
  (let [date-info (or (get-date-from-json-ld tree) (get-date-from-title-attr tree))
        ...
        description (let [og (et.rz.hub.scrapers.common/get-property tree "og:description")]
                      (if (str/blank? og) (or (:text date-info) "") og))
        title (subs description 0 (min 255 (count description)))
        image-url (et.rz.hub.scrapers.common/get-property tree "og:image")
