...
# the point. `make start` connects to a healthy hub if it finds one --
...
echo "Starting hub for e2e on :$DB_PORT (db: $DB_PATH)..."
...
trap 'stop_e2e_hub; rm -f .dev-server.lock' EXIT INT TERM
